...
def cmd_make_configs(ns):
    ...
    explicit_cfg = SimulationConfig(
        ...
        heat_report_freq=25000,
        density_report_freq=25000,
        ntebpreppercyc=equil_steps,
        ntebpercyc=equil_steps,
        ntprodpercyc=prod_steps,
        ebprepRestartFreq=50000,
        ebRestartFreq=50000,
        prodRestartFreq=100000,
        platform="CUDA",
        precision="mixed",
        cuda_device_index=0,
    ...
    gamd_dual_cfg = SimulationConfig(
        ...
        gamd_ramp_ns=0.5,
        gamd_start_stage="after_density",
        allow_dual_fallback=True,
        fallback_boost_mode="dihedral",
        cmd_ns=ns.cmd_ns,
        equil_ns_per_cycle=ns.equil_ns_per_cycle,
        prod_ns_per_cycle=ns.prod_ns_per_cycle,
        ntcmd=cmd_steps,
        ...
        cuda_precision="mixed",
        require_gpu=True,
        outdir="out_gamd_dual_opc_ff19sb",
    )

    gamd_dihedral_cfg = SimulationConfig(
        parmFile="topology/complex_ff19sb_opc.parm7",
        crdFile="topology/complex_ff19sb_opc.rst7",
        simType="protein.explicit",
        nbCutoff=10.0,
    ...
    gamd_dual_path = out_dir / "gamd_dual_opc_ff19sb.yaml"
    gamd_dihedral_path = out_dir / "gamd_dihedral_safe.yaml"
    explicit_path.write_text(explicit_cfg.to_yaml(), encoding="utf-8")
    implicit_path.write_text(implicit_cfg.to_yaml(), encoding="utf-8")
    gamd_dual_path.write_text(gamd_dual_cfg.to_yaml(), encoding="utf-8")
    gamd_dihedral_path.write_text(gamd_dihedral_cfg.to_yaml(), encoding="utf-8")
    print(f"Wrote: {explicit_path}")
    print(f"Wrote: {implicit_path}")
    print(f"Wrote: {gamd_dual_path}")
    print(f"Wrote: {gamd_dihedral_path}")
